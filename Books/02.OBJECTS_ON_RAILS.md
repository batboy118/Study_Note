...
이 경우 `Post.public_method`는 아직 작성되지 않은 `Post.new 메서드`를 나타내는 `Method`객체에 대한 참조를 가져온다. 정상적인 작동 중에, 블로그는 post 객체를 생성하기 위해 이 메서드 참조(Post.new와 동등한)를 사용할 것이다. 그러나, 이것은 클래스는 테스트할 때 모든 호출가능한 객체로 대체할 수 있다.
